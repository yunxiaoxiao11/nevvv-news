package com.nevvv.app.gateway.filter;
...
public class GatewayFilter implements GlobalFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        String requestUrl = exchange.getAttribute(ServerWebExchangeUtils.GATEWAY_REQUEST_URL_ATTR).toString();
        String[] split = requestUrl.split("/");
        log.info("前端访问的路径为:{}", requestUrl);
        //请求路径不是登录拒绝访问
        if (split[split.length - 1].equals("login_auth")) {
            //登录请求放行
        ...
        try {
            ...
            String redisToken = redisTemplate.opsForValue().get(tokenStr);
